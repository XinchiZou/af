...
def main():
    ...
    def _auto_linear_xlim(times_list, pad_ratio: float = 0):
    ...
    if x_right < 4:
        x_right = 4.0
    if 4 not in ticks:
        ticks = sorted(set(list(ticks) + [4]))
        if use_linear_x:
            labels = [str(int(t)) if abs(t - round(t)) < 1e-8 else f"{t:g}" for t in ticks]
        else:
            def _fmt_tick(x: float) -> str:
                ...
                if x >= 1:
                    return f"{x:.0f}"
                return f"{x:g}"
            labels = [_fmt_tick(t) for t in ticks]
    ...
    plt.savefig('efficiency_pareto_lastfm.png', format='png', dpi=300, bbox_inches='tight')
    plt.savefig('efficiency_pareto_lastfm.pdf', format='pdf', dpi=300, bbox_inches='tight')
    plt.show()
