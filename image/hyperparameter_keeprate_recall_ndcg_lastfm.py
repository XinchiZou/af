...
# --- 与 hyperparameter.py / hyperparameter_temp_recall_ndcg.py 保持一致的风格 ---
plt.rcParams.update({
    "font.family": "serif",
...
def plot_keeprate_sensitivity_dual_axis():
    """Keep rate 对 Recall/NDCG 的影响（双轴）。

    数据来自你最新截图：
    keep0.4: R=0.4094, N=0.2482
    keep0.8: R=0.4080, N=0.2478
    keep0.6: R=0.4085, N=0.2466
    keep0.2: R=0.4060, N=0.2453
    """
    ...
    # 配色：对齐 temperature 图风格（橙色柱 + 蓝色虚线X标记折线）
    ...
    def set_nice_yticks(
        ...
        # rounding：避免浮点误差导致出现 0.244999 之类
    ...
    # 标题
    ax_left.set_title('Last-FM', fontweight='semibold', fontsize=28, pad=12)

    plt.tight_layout()

    # 保存
    plt.savefig('hyperparameter_keeprate_recall_ndcg_lastfm.png', dpi=300, bbox_inches='tight')
    plt.savefig('hyperparameter_keeprate_recall_ndcg_lastfm.pdf', dpi=300, bbox_inches='tight')
    print('File saved: hyperparameter_keeprate_recall_ndcg_lastfm.png/.pdf')

    plt.show()


if __name__ == '__main__':
    plot_keeprate_sensitivity_dual_axis()
