...
def main():
    ...
    # 重要：横坐标范围固定为 7-21（并手动指定主刻度）
    use_linear_x = True
    x_left, x_right = 8, 20
    ticks = list(range(8, 21, 4))
    ...
    plt.savefig('efficiency_pareto_yelp.png', format='png', dpi=300, bbox_inches='tight')
    plt.savefig('efficiency_pareto_yelp.pdf', format='pdf', dpi=300, bbox_inches='tight')
    plt.show()
